...
function luvlsp.do_change(_, bufnr, tick, start, stop, stopped, bytes, codes, units)
  ...
  local edit = {range=range, text=text, rangeLength=units}
...
function luvlsp.do_open(bufnr)
  ...
  a.nvim_buf_attach(bufnr, false, {on_lines=function(...) luvlsp.do_change(...) end, utf_sizes=true})
end
